...
*/		        	
   ...
   /* private List<ContentPacketExtension> createParticipantOffer(Participant peer)
    ...
    protected void onMemberLeft(ChatRoomMember chatRoomMember)
    {
		...
		// Notify MUC by PrivateIQ message
	...
	// Notify the MUC by PrivateIQ message
	sendPrivateIQ(participant);
    ...
    boolean handleHoldRequest(String fromJid,
            String toBeHoldJid,
            boolean doHold)
	{
		Participant principal = findParticipantForRoomJid(fromJid);
     ...
     *            the participant
     */
    public void sendPrivateIQ(Participant participant) {
	if (isInTheRoom()) {
	    PrivateIQ privateIQ = new PrivateIQ();
	    privateIQ.setJid(this.getFocusRealJid());
	    privateIQ.setFrom(this.getFocusJid());
	    privateIQ.setTo(this.roomName);
	    privateIQ.setType(org.jivesoftware.smack.packet.IQ.Type.SET);
	    privateIQ.setJabberid(participant.getJabberid());
	    privateIQ.setAudioSupport(participant.hasAudioSupport());
	    privateIQ.setVideoSupport(participant.hasVideoSupport());
